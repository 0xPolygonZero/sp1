...
impl<F: PrimeField32, P: FieldParameters> FieldOpCols<F, P> {
    ...
    pub fn populate_sub_sub(
        ...
        debug_assert_eq!(&sum_with_result, &(a + &carry * &modulus));
        debug_assert!(carry < modulus);
    ...
    /// Populate for multiply-then-add: result = (a*b + c) mod modulus.
    /// Caller should only use this when the unreduced carry cannot overflow
    /// (i.e., a*b + c < modulus^2).
    pub fn populate_mul_add(
        &mut self,
        record: &mut impl ByteRecord,
        a: &BigUint,
        b: &BigUint,
        ...
        let carry = (&sum - &result) / &modulus;

        debug_assert!(result < modulus);
        debug_assert!(carry < modulus);
        debug_assert_eq!(&result + &carry * &modulus, sum);
        ...
        // Vanishing: (a*b + c) - result - carry*modulus = 0
        let p_op = &p_a * &p_b + &p_c;
        let p_vanishing: Polynomial<F> = &p_op - &p_result - &p_carry * &p_modulus;
...
impl<V: Copy, P: FieldParameters> FieldOpCols<V, P> {
    ...
    pub fn eval_sub_sub<AB: SP1AirBuilder<Var = V>>(
        &self,
        builder: &mut AB,
        a: &(impl Into<Polynomial<AB::Expr>> + Clone),
        b: &(impl Into<Polynomial<AB::Expr>> + Clone),
        ...
        Limbs<V, P::Limbs>: Copy,
    {
        ...
        let (p_a, p_result): (Polynomial<_>, Polynomial<_>) = (self.result.into(), p_a_param);
    ...
    pub fn eval_mul_add<AB: SP1AirBuilder<Var = V>>(
        &self,
        builder: &mut AB,
        a: &(impl Into<Polynomial<AB::Expr>> + Clone),
        b: &(impl Into<Polynomial<AB::Expr>> + Clone),
