    ...
    AB: SP1AirBuilder,
{
    fn eval(&self, builder: &mut AB) {
        ...
        for i in 0..10 {
            is_real = is_real.clone() + local.octet_num[i].into();
        }
        let is_initialize = local.octet_num[0];
        let is_compression = local.octet_num[1]
            + local.octet_num[2]
            + local.octet_num[3]
            + local.octet_num[4]
            + local.octet_num[5]
...
impl ShaCompressChip {
    fn eval_control_flow_flags<AB: SP1AirBuilder>(
        ...
        builder.assert_bool(octet_sum.clone());
        builder.when(is_real.clone()).assert_one(octet_sum);
        ...
        // Linda note: if is_last_row, it must be real with my update.
        builder.when_transition().when_not(local.is_last_row).assert_eq(local.shard, next.shard);
        builder.when_transition().when_not(local.is_last_row).assert_eq(local.clk, next.clk);
        builder.when_transition().when_not(local.is_last_row).assert_eq(local.w_ptr, next.w_ptr);
        builder.when_transition().when_not(local.is_last_row).assert_eq(local.h_ptr, next.h_ptr);
        ...
        builder
            ...
            .assert_one(next_is_real.clone());

        // Once the is_real flag is changed to false, it should not be changed back.
        builder.when_transition().when_not(is_real.clone()).assert_zero(next_is_real);
    ...
    fn eval_memory<AB: SP1AirBuilder>(
        ...
        builder: &mut AB,
        local: &ShaCompressCols<AB::Var>,
        is_initialize: AB::Var,
        is_compression: AB::Expr,
        is_finalize: AB::Var,
    ) {
        builder.eval_memory_access(
            ...
            local.mem_addr,
            &local.mem,
            is_initialize + is_compression.clone() + is_finalize,
        );
        ...
        builder.when(is_finalize.clone()).assert_eq(
            local.mem_addr,
            local.h_ptr + cycle_step.clone() * AB::Expr::from_canonical_u32(4),
        );
        ...
        builder.when(is_compression).assert_word_eq(*local.mem.prev_value(), *local.mem.value());

        // In the finalize phase, verify that the correct value is written to memory.
        builder.when(is_finalize).assert_word_eq(*local.mem.value(), local.finalize_add.value);
    }
    ...
    fn eval_compression_ops<AB: SP1AirBuilder>(
        ...
        // Calculate not e.
        NotOperation::<AB::F>::eval(builder, local.e, local.e_not, is_compression.clone());
        ...
        // b := a
        // a := temp1 + temp2
        builder.when_transition().when(is_compression.clone()).assert_word_eq(next.h, local.g);
        builder.when_transition().when(is_compression.clone()).assert_word_eq(next.g, local.f);
        builder.when_transition().when(is_compression.clone()).assert_word_eq(next.f, local.e);
        builder
            .when_transition()
            .when(is_compression.clone())
            .assert_word_eq(next.e, local.d_add_temp1.value);
        builder.when_transition().when(is_compression.clone()).assert_word_eq(next.d, local.c);
        builder.when_transition().when(is_compression.clone()).assert_word_eq(next.c, local.b);
        builder.when_transition().when(is_compression.clone()).assert_word_eq(next.b, local.a);
        builder
            .when_transition()
            .when(is_compression)
